...
import com.jeta.jgoodies.forms.layout.FormSpec;

/**
 * The UIDirector for the MainFrame
 */
public class MainPanelUIDirector implements UIDirector {
	/**
	 ...
	 * ctor
	 */
	public MainPanelUIDirector(MainPanel frame) {
		m_frame = frame;
	}
	...
	public void updateComponents(java.util.EventObject evt) {
		m_frame.enableComponent(MainPanelNames.ID_CREATE_FORM, true);
		m_frame.enableComponent(MainPanelNames.ID_OPEN_FORM, true);

		boolean has_editor = (m_frame.getCurrentEditor() != null);
		boolean has_undo = false;
		boolean has_redo = false;
		if(has_editor){
			has_undo = m_frame.getCurrentEditor().getUndoManager().canUndo();
			has_redo = m_frame.getCurrentEditor().getUndoManager().canRedo();
		}


		m_frame.enableComponent(MainPanelNames.ID_SHOW_FORM, has_editor);
		m_frame.enableComponent(MainPanelNames.ID_SAVE_FORM, has_editor);
		m_frame.enableComponent(MainPanelNames.ID_SAVE_FORM_AS, has_editor);
		m_frame.enableComponent(MainPanelNames.ID_CLOSE_FORM, has_editor);
		m_frame.enableComponent(TSComponentNames.ID_UNDO, has_undo);
		m_frame.enableComponent(TSComponentNames.ID_REDO, has_redo);

		m_frame.enableComponent(MainPanelNames.ID_FORWARD_ENGINEER, has_editor);
		...
		m_frame.enableComponent(FormEditorNames.ID_MOVE_LEFT, can_span);
		m_frame.enableComponent(FormEditorNames.ID_MOVE_RIGHT, can_span);
		m_frame.enableComponent(FormEditorNames.ID_MOVE_UP, can_span);
		m_frame.enableComponent(FormEditorNames.ID_MOVE_DOWN, can_span);
		
		m_frame.enableComponent(FormEditorNames.ID_DELETE_COMPONENT, can_span);
