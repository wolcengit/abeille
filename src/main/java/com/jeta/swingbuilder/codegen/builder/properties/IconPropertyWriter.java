...
public class IconPropertyWriter implements PropertyWriter {
			...
			if (value instanceof IconProperty) {
				...
				if (path != null && path.length() > 0) {
					...
					if (write != null) {
						...
						ms.addParameter(createIconExpression(path));
	...
	public static MethodExpression createIconExpression(String iconName) {
		MethodExpression expr = new MethodExpression("loadImage");
		expr.addParameter(new StringExpression(iconName));
		return expr;
	}
